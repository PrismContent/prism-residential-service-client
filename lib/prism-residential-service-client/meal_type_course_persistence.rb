    ...
    class << self
      ...
      def save(meal_type_course)
        ...
        if response.timed_out?
          meal_type_course.send("service_errors=".to_sym, "Connection timeout. Please try again soon.")
          return false
        end
        ...
        case response.code
          ...
          when 201
            ...
            return true
          when 0 
            meal_type_course.send("service_errors=".to_sym, "Unable to connect to service. Please try again soon.")
            return false  
          else
            meal_type_course.send("service_errors=".to_sym, error_from(response))
            return false
        end
      end
      ...
      def collection_from(response)
        attrs = json_data(response)['meal_type_courses']
        attrs.map{|attr| ResidentialService::MealTypeCourse.new(attr) }
      end

      def instance_from(response)
        attr = json_data(response)['meal_type_course']
        ResidentialService::MealTypeCourse.new attr
      end
